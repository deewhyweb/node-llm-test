...
prompt.extendExpect();
const usePrompt = prompt.usePrompt;
...
describe('Parasol model training tests', () => {
  test('should pass when model is asked what an insurance policy', async () => {
    await expect(await usePrompt('What is an insurance policy')).toMatchSemanticSimilarity(
  ...
  test('should pass when model is asked what is Parasol insurance', async () => {
    await expect(await usePrompt('What is Parasol Insurance?')).toMatchSemanticSimilarity(
